			...
			<el-form class="center-form-pv" :style='{"padding":"0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","margin":"0 20px 20px 0","borderRadius":"3px","alignItems":"center","textAlign":"left","background":"#fff","display":"flex","width":"auto","fontSize":"inherit","order":"2"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">科室分类</label>
						<el-input v-model="searchForm.keshifenlei" placeholder="科室分类" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","padding":"10px","margin":"0px 0 20px","color":"#fff","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"3px","flexWrap":"wrap","background":"#fff","flex":"1","width":"100%","fontSize":"inherit","order":"2"}'>
					<el-button class="add" v-if="isAuth('keshifenlei','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('keshifenlei','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"0 0 2px","boxShadow":"0 1px 3px 0px rgba(115,108,203,.23)","borderColor":"rgba(252,234,238,1)","margin":"0 0 20px","borderRadius":"3px","background":"rgba(255,255,255,1)","borderWidth":"0px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('keshifenlei','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('keshifenlei','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.keshifenlei!='' && this.searchForm.keshifenlei!=undefined){
            params['keshifenlei'] = '%' + this.searchForm.keshifenlei + '%'
          }
      this.$http({
        url: "keshifenlei/page",
        method: "get",
        params: params
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	keshifenleistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'keshifenlei/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
