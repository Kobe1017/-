<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","fontSize":"14px","color":"#666","background":"none","height":"100%"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yuyueguahao','就诊')" @click="jiuzhenxinxiCrossAddOrUpdateHandler(scope.row,'cross','','[1]','已就诊')" type="success">
		...
		<jiuzhenxinxi-cross-add-or-update v-if="jiuzhenxinxiCrossAddOrUpdateFlag" :parent="this" ref="jiuzhenxinxiCrossaddOrUpdate"></jiuzhenxinxi-cross-add-or-update>
...
import jiuzhenxinxiCrossAddOrUpdate from "../jiuzhenxinxi/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				jiuzhenxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			jiuzhenxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.jiuzhenxinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuyueguahao');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jiuzhenxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jiuzhenxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
